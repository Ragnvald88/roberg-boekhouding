...
async def open_werkdag_dialog(on_save=None, werkdag=None, prefill: dict | None = None):
    ...
    # Initial datum: edit-mode → werkdag.datum, prefill → prefill['datum'], else today
    if is_edit:
        ...
        with ui.card().classes('werkdag-dialog-card q-pa-none'):
            ...
            with ui.column().classes('werkdag-dialog-body w-full gap-4'):
                ...
                with ui.column().classes('settings-section w-full'):
                    ...
                    # Locatie-row + caption (verschijnt als klant locaties heeft)
                    locatie_row = ui.column().classes('w-full gap-1')
                ...
                with ui.row().classes('werkdag-totaal-strook'):
                    ...
                    bedrag_label = ui.label(format_euro(0)).classes(
            ...
            async def save(and_new: bool = False):
                ...
                try:
                    if is_edit:
                        ...
                        # Acceptabel voor MVP — confirm_expected gebruikt
                        # pattern-defaults voor uren/tarief/km/code. Daarom
                        # zijn die velden disabled in de UI (zie hierboven).
