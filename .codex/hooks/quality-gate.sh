...
# Only run if code files were likely changed (check git status).
# .py/.html cover Python + Jinja templates; .sql for schema/migrations;
# .css for cascade-discipline tests; .js for any frontend hooks.
if ! git diff --name-only HEAD 2>/dev/null | grep -qE '\.(py|html|sql|css|js)$'; then
  # No code changes — skip test enforcement
  exit 0
fi
